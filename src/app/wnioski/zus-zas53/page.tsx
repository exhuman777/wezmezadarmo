...
type RodzajNiezdolnosci = 'nie-dotyczy' | 'choroba-zawodowa' | 'wypadek-przy-pracy' | 'wypadek-w-drodze';
...
interface Zas53Data {
  ...
  dataUstaniaUbezpieczenia: string; // jesli po ustaniu
  rodzajNiezdolnosci: RodzajNiezdolnosci;
}

type Step = 'wnioskodawca' | 'platnik' | 'zasilek' | 'podglad' | 'done';
...
const EMPTY: Zas53Data = {
  ...
  rodzajNiezdolnosci: 'nie-dotyczy',
};

const STEPS: { key: Step; label: string }[] = [
  { key: 'wnioskodawca', label: 'Twoje dane' },
  { key: 'platnik', label: 'Platnik skladek' },
  { key: 'zasilek', label: 'Wniosek o zasilek' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-0)' }}>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '48px 24px 80px' }}>
        ...
        <h1 className="display" style={{ fontSize: 'clamp(26px, 4vw, 38px)', marginBottom: 8 }}>Zasilek chorobowy</h1>
        <p style={{ fontSize: 14, color: 'var(--color-text-3)', marginBottom: 32 }}>Wniosek ZAS-53 skladany bezposrednio do ZUS</p>
        ...
        <div style={{
          ...
          <p style={{ marginBottom: 14 }}>Ten formularz skladaja bezposrednio do ZUS: przedsiebiorcy prowadzacy dzialalnosc, osoby wspolpracujace, osoby korzystajace z ulgi na start, duchowni, marynarze oraz osoby niezdolne do pracy po ustaniu zatrudnienia. Pracownicy etatowi skladaja wniosek przez pracodawce (nie ZAS-53).</p>
          <p style={{ fontWeight: 600, color: 'var(--color-text-1)', marginBottom: 6, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Gdzie zlozyc</p>
        ...
        {step === 'platnik' && <StepPlatnik data={data} update={update} onBack={() => setStep('wnioskodawca')} onNext={() => setStep('zasilek')} />}
        {step === 'zasilek' && <StepZasilek data={data} update={update} onBack={() => setStep('platnik')} onNext={() => setStep('podglad')} />}
        {step === 'podglad' && (
          <StepPodglad data={data} onBack={() => setStep('zasilek')} onDone={() => setStep('done')}
...
function StepPlatnik({ data, update, onBack, onNext }: { data: Zas53Data; update: (k: keyof Zas53Data, v: string) => void; onBack: () => void; onNext: () => void }) {
  return (
    <div>
...
function StepZasilek({ data, update, onBack, onNext }: { data: Zas53Data; update: (k: keyof Zas53Data, v: string) => void; onBack: () => void; onNext: () => void }) {
  const valid = data.dataOd.trim().length > 0 && data.dataDo.trim().length > 0;
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 500, color: 'var(--color-text-1)', marginBottom: 24, letterSpacing: '-0.01em' }}>Wniosek o przyznanie zasilku chorobowego</h2>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <Field label="Seria i numer zaswiadczenia lekarskiego (e-ZLA)" hint="Opcjonalnie, jesli pamietasz. Zaswiadczenie elektroniczne trafia do ZUS automatycznie.">
        ...
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--color-text-1)', marginBottom: 8 }}>
            Wniosek dotyczy okresu niezdolnosci do pracy:
          </label>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { v: 'w-trakcie', label: 'W okresie ubezpieczenia chorobowego / wypadkowego' },
              { v: 'po-ustaniu', label: 'Po ustaniu tytulu ubezpieczenia chorobowego / wypadkowego' },
            ].map(opt => (
        ...
        <div>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--color-text-1)', marginBottom: 8 }}>
            Wniosek dotyczy niezdolnosci z powodu:
          </label>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {(Object.entries(RODZAJ_LABELS) as [RodzajNiezdolnosci, string][]).map(([k, v]) => (
              <label key={k} style={{
                display: 'flex', gap: 10, alignItems: 'center',
                padding: '12px 14px',
                background: data.rodzajNiezdolnosci === k ? 'color-mix(in srgb, var(--color-accent) 8%, var(--color-bg-1))' : 'var(--color-bg-1)',
                border: `1px solid ${data.rodzajNiezdolnosci === k ? 'var(--color-accent)' : 'var(--color-border)'}`,
                borderRadius: 8, cursor: 'pointer',
              }}>
                <input type="radio" name="rodzaj" value={k} checked={data.rodzajNiezdolnosci === k} onChange={() => update('rodzajNiezdolnosci', k)} style={{ accentColor: 'var(--color-accent)' }} />
        ...
        {(data.rodzajNiezdolnosci === 'choroba-zawodowa' || data.rodzajNiezdolnosci === 'wypadek-przy-pracy') && (
          <div style={{ padding: '12px 16px', background: 'color-mix(in srgb, var(--color-accent) 6%, var(--color-bg-1))', border: '1px solid var(--color-amber-border)', borderRadius: 8, fontSize: 12, color: 'var(--color-text-2)', lineHeight: 1.6 }}>
            {data.rodzajNiezdolnosci === 'choroba-zawodowa'
              ? 'Dolacz do wniosku: decyzje o stwierdzeniu choroby zawodowej i zaswiadczenie lekarskie, ze niezdolnosc jest spowodowana ta choroba.'
              : 'Wyplata zasilku z tytulu wypadku nastapi po uznaniu zdarzenia za wypadek przy pracy. Dolacz protokol powypadkowy lub karte wypadku.'}
          </div>
        )}
        ...
        <div style={{ padding: '12px 16px', background: 'var(--color-bg-1)', border: '1px solid var(--color-border)', borderRadius: 8, fontSize: 12, color: 'var(--color-text-3)', lineHeight: 1.6 }}>
          <p style={{ fontWeight: 600, color: 'var(--color-text-2)', marginBottom: 4 }}>Wysokosc zasilku</p>
          <p>80% podstawy wymiaru. 100% gdy: niezdolnosc w ciazy, wypadek przy pracy lub w drodze, choroba zawodowa, dawstwo komorek i tkanek. Maksymalny okres: 182 dni (270 dni dla gruzlicy lub w ciazy).</p>
        </div>
      </div>
      <NavButtons onBack={onBack} onNext={onNext} nextDisabled={!valid} nextLabel="Podglad wniosku" />
    </div>
  );
}
...
function StepPodglad({ data, onBack, onDone, onDownloadPdf, downloadingPdf, onCopy, copied }: {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 500, color: 'var(--color-text-1)', marginBottom: 24, letterSpacing: '-0.01em' }}>Podglad wniosku ZAS-53</h2>
      ...
      <div style={{ marginTop: 24, padding: '20px 24px', background: 'var(--color-bg-1)', border: '1px solid var(--color-border)', borderRadius: 10, fontSize: 13, color: 'var(--color-text-2)', boxShadow: 'var(--shadow-1)' }}>
        <p style={{ fontWeight: 600, color: 'var(--color-text-1)', marginBottom: 12, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Jak zlozyc ZAS-53</p>
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 14, color: 'var(--color-text-2)', marginBottom: 32, lineHeight: 1.7 }}>
        Pobierz oryginalny formularz ZAS-53 z zus.pl, przepisz dane i zloz w ZUS lub przez eZUS.
      </p>
...
function buildOutput(d: Zas53Data): string {
  const rodzajLabel = RODZAJ_LABELS[d.rodzajNiezdolnosci];
  const ustanie = d.okresUbezpieczenia === 'po-ustaniu'
    ? `Po ustaniu tytulu ubezpieczenia (data ustania: ${d.dataUstaniaUbezpieczenia})`
...
Okres niezdolnosci od: ${d.dataOd}
Okres niezdolnosci do: ${d.dataDo}
Seria i numer e-ZLA: ${d.numerEzla || '-- nie podano'}

Wniosek dotyczy okresu: ${ustanie}

Rodzaj niezdolnosci: ${rodzajLabel}
...
========================================
INSTRUKCJA:
1. Online: zaloguj sie do eZUS: https://www.zus.pl/ezus/logowanie?jezyk=pl
2. Lub pobierz oryginalny ZAS-53: https://www.zus.pl/wzory-formularzy
3. Przepisz dane WIELKIMI LITERAMI
4. Zloz w oddziale ZUS lub wyslij pocztą

Termin: do 6 miesiecy od ostatniego dnia zwolnienia.
