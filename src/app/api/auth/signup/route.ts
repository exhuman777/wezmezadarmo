...
export async function POST(request: NextRequest) {
  ...
  // KLUCZOWE: nie uzywamy linkData.action_link (uzywa hash fragment ktorego serwer nie widzi).
  // Budujemy wlasny URL z hashed_token -> nasz callback robi verifyOtp.
  const hashedToken = linkData?.properties?.hashed_token;
  const confirmUrl = hashedToken
    ? `${siteUrl}/auth/callback?token_hash=${hashedToken}&type=signup&next=/panel`
    ...
    try {
      await resend.emails.send({
        ...
        html: [
          ...
          `<a href="${confirmUrl}" style="display:inline-block;padding:12px 28px;background:#0d5036;color:#fff;border-radius:999px;text-decoration:none;font-size:15px;font-weight:500">`,
          'Potwierdź e-mail</a>',
